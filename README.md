...
아래 파일들은 GitHub에 올리지 않고, 각 팀원이 별도로 내려받아 지정 위치에 넣는 방식입니다.

### 로그 파일

아래 3개 파일을 `backend/` 폴더에 넣어야 합니다.
...
아래 2개 파일을 `frontend/public/` 폴더에 넣어야 합니다.
...
## requirements.txt로 한 번에 가능한가?

Python 패키지만 설치하는 것은 `requirements.txt`로 가능합니다.  
하지만 현재 프로젝트는 프론트엔드 실행을 위해 `nodejs`와 `npm`도 필요하므로, 전체 환경을 한 번에 맞추려면 `requirements.txt`만으로는 부족합니다.
...
즉, 팀원 입장에서는 `setup.bat` 한 번이면 됩니다.
...
이 `GUI` 폴더를 별도 저장소로 올리는 것을 권장합니다.
...
- `frontend/node_modules`는 저장소에 포함하지 않습니다.
- 로그 파일과 `.mov` 파일은 저장소에 포함하지 않도록 `.gitignore` 처리되어 있습니다.
